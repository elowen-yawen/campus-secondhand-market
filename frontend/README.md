...
## 十三、常见问题排查 (Troubleshooting)

### 13.1 后端连接 404
- **现象**：访问 `http://127.0.0.1:5000` 提示 404。
- **原因**：后端未定义根路由 `/`。
- **修复**：已在 `back/src/main.py` 中添加了根路由映射，现在访问会返回服务状态 JSON。

### 13.2 接口请求 401 Unauthorized
- **现象**：Network 面板显示部分请求（如未读消息刷新）为 401。
- **原因**：请求未携带有效的 `Authorization` Token。
- **修复**：确保所有请求通过 `api/index.js` 封装的 `get/post` 方法发起。已修复 `tab-bar.vue` 中的硬编码请求。

### 13.3 状态码校验错误
- **现象**：后端返回成功，但前端提示“请求失败”。
- **原因**：后端成功码约定为 `0`，部分旧代码可能在校验 `200`。
- **修复**：检查并统一校验 `res.data.code === 0`。

### 13.4 开发环境地址配置
- **文件**：`frontend/api/index.js`
- **配置**：`export const BASE_URL = 'http://127.0.0.1:5000'`
- **注意**：真机调试需将 `127.0.0.1` 替换为电脑局域网 IP。
